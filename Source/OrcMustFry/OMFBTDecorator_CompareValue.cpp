...
#include "OMFBTDecorator_CompareValue.h"
...
bool UOMFBTDecorator_CompareValue::CalculateRawConditionValue(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory) const
{
...
bool UOMFBTDecorator_CompareValue::EvaluateOnBlackboard(const UBlackboardComponent& BlackboardComp) const
{
	bool bResult = false;
...
EBlackboardNotificationResult UOMFBTDecorator_CompareValue::OnBlackboardKeyValueChange(const UBlackboardComponent& Blackboard, FBlackboard::FKey ChangedKeyID)
{
...
void UOMFBTDecorator_CompareValue::DescribeRuntimeValues(const UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, EBTDescriptionVerbosity::Type Verbosity, TArray<FString>& Values) const
{
...
FString UOMFBTDecorator_CompareValue::GetStaticDescription() const
{
...
void UOMFBTDecorator_CompareValue::BuildDescription()
{
...
void UOMFBTDecorator_CompareValue::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (ChangedPropName == GET_MEMBER_NAME_CHECKED(UOMFBTDecorator_CompareValue, BlackboardKey.SelectedKeyName))
	{
	...
	if (ChangedPropName == GET_MEMBER_NAME_CHECKED(UOMFBTDecorator_CompareValue, BasicOperation))
	{
	...
	else if (ChangedPropName == GET_MEMBER_NAME_CHECKED(UOMFBTDecorator_CompareValue, ArithmeticOperation))
	{
	...
	else if (ChangedPropName == GET_MEMBER_NAME_CHECKED(UOMFBTDecorator_CompareValue, TextOperation))
	{
...
void UOMFBTDecorator_CompareValue::InitializeFromAsset(UBehaviorTree& Asset)
{
